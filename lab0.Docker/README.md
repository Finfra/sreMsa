...
**첫날 실습은 내 Windows 에 Docker Desktop 을 설치하고 진행한다.** VM 이나 Linux 를 거치지 않고 익숙한 환경에서 컨테이너를 먼저 다뤄 보기 위한 것이다.
...
설치를 마치면 아래 두 실습을 차례로 한다. 실습 문서는 **실습 소스**(`다운로드\sreMsa\`) 안에 있다.

| 순서  | 실습                         | 문서                                         |
| :---: | :--------------------------- | :------------------------------------------- |
| **1** | 컨테이너 사용 — `docker run` | `다운로드\sreMsa\lab0.Docker\1.dockerRun\`   |
| **2** | 이미지 제작 — `docker build` | `다운로드\sreMsa\lab0.Docker\2.dockerBuild\` |

* 1번은 **남이 만든 이미지를 받아 쓰는** 법, 2번은 **내가 이미지를 만드는** 법이다.
* 2번의 예제는 [github.com/Finfra/dockers](https://github.com/Finfra/dockers) 의 `ubuntu_basic` 을 따른다.
...
* `WSL 2를 실행하려면 커널 구성 요소 업데이트가 필요합니다` 가 나오면 msi 가 제대로 설치되지 않은 것이다. `/quiet` 를 빼고 다시 실행해 화면을 본다.
...
> ⚠️ **`다시 시작`으로 재부팅한다.** Windows 11 의 **빠른 시작**이 켜져 있으면 `시스템 종료` 후 전원을 켜는 것은
> 재부팅이 아니라 최대 절전에서 복귀하는 것이라 `bcdedit` 변경이 **적용되지 않는다.**
> 위의 `shutdown -r -t 0` 을 쓰거나 `시작 → 전원 → 다시 시작` 을 고른다.
...
* `True` 면 **메모리 무결성 말고도 되살리는 것이 더 있다** — Windows 11 은 `Credential Guard`·`VBS`·
  Windows 기능(`가상 머신 플랫폼` 등)이 같은 일을 한다. 원인을 짚는 절차는
  [0.pc_setting/README.md](../lab1.Kubespray/pc/0.pc_setting/README.md) 의 **"그래도 `True` 일 때"** 절에 있다.
* ⚠️ **VirtualBox 7 은 Hyper-V 가 남아 있어도 실패하지 않고 10배 느려진다**(NEM 폴백).
  그래서 증상이 "실패" 가 아니라 **"부팅 타임아웃"** 으로 나타나 디스크를 의심하게 된다.
  2일차에 VM 이 유난히 느리면 이것부터 의심한다.
...
| `WSL 2를 실행하려면 커널 업데이트가 필요합니다`         | 2단계 msi 가 설치되지 않았다. `/quiet` 없이 다시 실행해 화면을 본다                                                                   |
...
| **껐는데도 `HypervisorPresent` 가 `True`**              | Credential Guard·VBS·Windows 기능이 되살리고 있다 — [0.pc_setting](../lab1.Kubespray/pc/0.pc_setting/README.md) "그래도 `True` 일 때" |
| **2일차 VM 이 뜨긴 하는데 매우 느리다**                 | VirtualBox 가 NEM 으로 폴백했다. 같은 원인이며 위 항목과 함께 본다                                                                    |

# 참고 — 2일차가 끝내 막혔을 때

회사 정책으로 VirtualBox 를 설치하지 못하거나 관리자 권한이 없어 2일차 환경을 만들 수 없다면, **Docker Desktop 위에 kind 로 Kubernetes 를 올려** 실습을 이어 갈 수 있다.
