...
* **주의 : CE 의 최대 개수 상한은 5 입니다.** `max` 에 **6 이상**을 주면 배포가 거부됩니다.

> ```
> Function 'nodeinfo' failed to deploy with status code: 400
> ```
>
> 실측으로 `max=5` 는 `202 Accepted`, `max=6`·`max=20` 은 둘 다 `400` 이었습니다.
> 이 오류는 한 줄로만 지나가므로 **못 보고 넘기기 쉽습니다.** 배포 후 `READY` 가 그대로면 이것부터 의심하십시오.
...
kubectl get deploy -n openfaas-fn nodeinfo
```
* `min=2` 로 주면 부하가 없어도 2개가 유지됩니다. 실측에서 `2/2` 를 확인했습니다.
* cf) **이미 떠 있는 함수에도 라벨은 그대로 반영됩니다.** 지웠다 다시 배포할 필요가 없습니다 — 실측에서 라벨을 덧붙인 직후 `1/2`, 5초 뒤 `2/2` 가 됐습니다.
* **콜드 스타트를 줄이려고 최소 개수를 올리는 것**이 강의에서 말한 "최소 인스턴스 상시 유지" 입니다.
