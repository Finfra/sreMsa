...
## 2. 노드가 이 저장소를 믿게 하기 [i1에서 실행]

* 우리 저장소는 https 가 아니라 http 라서, 그냥 두면 노드가 이미지를 받기를 거부합니다.
* **주의 : 이 작업은 `i1` 에서 실행합니다.** 노드 3대에 접속할 키가 i1 에만 있습니다.
    - vm01 에서 실행하면 `ubuntu@vm02: Permission denied (publickey,password).` 로 막힙니다. vm01 에는 다른 노드로 갈 개인키가 없습니다.
...
* **처음 한 번은 1분 안팎 걸립니다**(실측 45~62초). 베이스 이미지(`node:20-alpine`·`of-watchdog`)를
  받아 오기 때문이며, VM 을 갓 만든 직후일수록 깁니다.
  두 번째부터는 캐시가 남아 **10~20초**, 코드만 고친 재빌드도 그 정도입니다. 멈춘 것이 아니니 기다리십시오.
