...
* **이 문서만 마치면** VM 을 만들 준비가 끝난다. 그 다음은
  [Install_Kubernetes_only_PC.md](Install_Kubernetes_only_PC.md) **2장** 부터 이어서 진행한다.
...
> ⚠️ **`docker/` 는 이 단계에서 설치하지 않는다.** VM 안의 Ubuntu 에 까는 것이며 [Install_Kubernetes_only_PC.md](Install_Kubernetes_only_PC.md) 11장에서 쓴다.
...
이 실습은 Hyper-V 를 **끈 상태로 끝까지** 진행한다 — 컨테이너 실습도 VM 안에서 하므로([Install_Kubernetes_only_PC.md](Install_Kubernetes_only_PC.md) 11장) 중간에 다시 켤 일이 없다.
...
→ [Install_Kubernetes_only_PC.md](Install_Kubernetes_only_PC.md) 의 **2. VM 만들기** 로 이어서 진행한다.
