...
SRC="$(cd "$(dirname "$0")" && pwd)"          # 공유 폴더 안의 원본 (cf_inVm/inner)
INVM="$(cd "$SRC/.." && pwd)"                 # cf_inVm
# 프로비저닝 스크립트(common.sh·node.sh)는 이웃 폴더 1.pc.byVagrant 것을 그대로 쓴다.
# 복제하지 않으므로 1.pc.byVagrant 를 고치면 이쪽에도 반영된다.
export SREMSA_LOCAL="$(cd "$INVM/../1.pc.byVagrant" && pwd)"
...
echo "    cd ~/kubespray"
echo "    SREMSA_CP_COUNT=1 bash /sreMsa/lab1.Kubespray/pc/2.pc.InstanceForKubernetes/doMakeInventory.sh"
