...
#
# AWS 경로(aws/3.aws.InstanceForKubernetes/README.md 5~6단계)와 같은 내용이되
# 로컬에서는 한 가지가 반드시 다르다.
#
...
#
# ⚠️ SREMSA_CP_COUNT 로 덮어쓸 수 있다 — cf_inVm(중첩)이 이것을 쓴다.
...
#    (1.pc.byVagrant 는 노드가 2 vCPU 라 2대로 두어도 15분 20초에 완주한다 — 기본값 유지)
