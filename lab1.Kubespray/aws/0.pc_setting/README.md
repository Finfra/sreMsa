...
* 로컬 PC 방식과 달리 **복구용 VM 백업이 필요 없다.** 내 PC 에 VM 을 만들지 않기 때문이다.
...
**파일명에 번호를 붙이지 않는다.** 설치 순서는 아래 표의 ①②③④ 가 정한다.

| 순서  | `_prgs` 안의 파일                 |          크기 | 용도                                          |
| :---: | :-------------------------------- | ------------: | :-------------------------------------------- |
| **①** | `VSCodeUserSetup-x64-1.137.0.exe` |        224 MB | VS Code — YAML·매니페스트 편집용              |
| **②** | `Git-2.55.0.5-64-bit.exe`         |         62 MB | Git for Windows — **Git Bash 를 SSH 로 쓴다** |
| **③** | `putty-64bit-installer.msi`       |          4 MB | PuTTY·PuTTYgen·pscp — Windows 표준 SSH 도구   |
| **④** | `DockerDesktopInstaller.exe`      |        600 MB | Docker Desktop — 로컬에서 이미지 빌드·테스트  |
|   —   | `SHA256SUMS.txt`                  |             — | 무결성 검증용 체크섬                          |
|       | 합계                              | **약 890 MB** |                                               |

설치 옵션은 전부 기본값 그대로 둔다. **④ Docker Desktop 은 WSL2 를 먼저 켠 뒤 설치한다** — 아래 절에 순서가 있다.
